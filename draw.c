...
int top(double a, double b, double c) {
  if (a >= b && a >= c) return 0;
  if (c >= a && c >= b) return 2;
  if (b >= c && b >= a) return 1;
  else return -1;
...
int bottom(double a, double b, double c) {
  if (a <= b && a <= c) return 0;
  if (c <= a && c <= b) return 2;
  if (b <= c && b <= a) return 1;
  else return -1;
...
void scanline_convert( struct matrix *points, int i, screen s, zbuffer zb, color c) {
    ...
    double xb,yb,zbot, xm,ym,zm, xt,yt,zt; //bottom, middle, top coords
    int t,m,b; //top middle bottom indices
    double y, x0,x1, z0,z1, dx0,dx1,dx1flip, dz0,dz1,dz1flip;
      ...
      yb = points->m[1][i+b];
      zbot = points->m[2][i+b];
      //current points (start from bottom)
      x0 = xb;
      x1 = xb;
      z0 = zbot;
      z1 = zbot;
      ...
      while (y < yt) {
        ...
        if (y == ym) {
          ...
          dz1 = dz1flip;
          x1 = xm;
          z1 = zm; //to prevent floating point inaccuracies
        }
      }
}
